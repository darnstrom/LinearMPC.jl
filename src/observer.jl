...
function codegen(observer::OffsetFreeObserver, mpc::Union{MPC,ExplicitMPC}, fh, fsrc)
    ...
    if mpc.nl > 0 && mpc.np > 0
        @printf(fh, "int mpc_compute_control_observer(c_float* control, c_float* observer_state, c_float* reference, c_float* measured_disturbance, c_float* linear_cost, c_float* affine_parameter);\n")
    elseif mpc.nl > 0
...
""")
    ...
    if mpc.nl > 0 && mpc.np > 0
        write(fsrc, """
int mpc_compute_control_observer(c_float* control, c_float* observer_state, c_float* reference, c_float* measured_disturbance, c_float* linear_cost, c_float* affine_parameter){
    ...
    return mpc_compute_control(control, state, reference, disturbance, linear_cost, affine_parameter);
}
""")
    elseif mpc.nl > 0
        write(fsrc, """
