...
function Model(F,G,Gd,C,Dd;Ts=-1.0, offset=zeros(0),true_dynamics=nothing)
    Model(F,G;Gd,offset,C,Dd,Ts,true_dynamics)
end

function Model(F,G;Ts=-1.0, C = zeros(0,0), Gd = zeros(0,0), offset=zeros(0), Dd = zeros(0,0), wmin=zeros(0), wmax=zeros(0), true_dynamics=nothing)
    ...
    Model(float(F),float(G),float(Gd), float(offset),float(wmin), float(wmax), float(C),float(Dd), true_dynamics,
          nx,nu,ny,nd,Ts,Labels(nx,nu,ny,nd))
end

function Model(A,B,Ts; Bd = zeros(0,0), C = zeros(0,0), Dd = zeros(0,0), offset=zeros(0), true_dynamics=nothing)
    dims = size(B);
    ...
    return Model(F,G;Ts,Gd,C,Dd,offset,true_dynamics=f)
end

function Model(A,B,Bd,C,Dd,Ts::AbstractFloat;offset=zeros(0), true_dynamics=nothing)
    Model(A,B,Ts;Bd,C,Dd,offset, true_dynamics)
end
