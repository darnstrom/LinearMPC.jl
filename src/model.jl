...
function Model(F,G,Gd,C,Dd;Ts=-1.0, offset=zeros(0))
    Model(F,G;Gd,offset,C,Dd,Ts)
end

function Model(F,G;Ts=-1.0, C = zeros(0,0), Gd = zeros(0,0), offset=zeros(0), Dd = zeros(0,0), wmin=zeros(0), wmax=zeros(0))
    ...
    Model(float(F),float(G),float(Gd), float(offset),float(wmin), float(wmax), float(C),float(Dd),
          nx,nu,ny,nd,Ts,Labels(nx,nu,ny,nd))
end

function Model(A,B,Ts; Bd = zeros(0,0), C = zeros(0,0), Dd = zeros(0,0), offset=zeros(0))
    dims = size(B);
    ...
    return Model(F,G;Ts,Gd,C,Dd,offset)
end

function Model(A,B,Bd,C,Dd,Ts::AbstractFloat;offset=zeros(0))
    Model(A,B,Ts;Bd,C,Dd,offset)
end
