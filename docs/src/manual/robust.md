...
<div class="lang-switcher-content active" data-lang="julia"><pre><code class="language-julia">using LinearMPC, Plots
...
plot!(sim_nominal.ys[1,:], xlabel="Time step", ylabel="Position [m]", label="Nominal MPC")</code></pre></div>
...
plt.legend(); plt.show()</code></pre></div>
</div>
```

```@example robust_mpc
using LinearMPC,Plots # hide

F,G = [1 0.1; 0 1], [0.005;0.1;;] # double integrator with Ts=0.1 # hide
true_dynamics = (x,u,d) -> F*x + G*u+0.01*(rand(2).-0.5); # hide

mpc_nominal = LinearMPC.MPC(F,G;Ts=0.1,Np=25,C=[1 0;]) # hide
set_bounds!(mpc_nominal;umin=[-0.2],umax=[0.2],ymin=[-0.5],ymax=[0.5]) # hide

sim_nominal = Simulation(true_dynamics,mpc_nominal;r=[0.5]) # hide
hline([0.5],label="Constraint bound", linestyle=:dash) # hide
plot!(sim_nominal.ys[1,:],xlabel="Time step", ylabel="Position [m]",label="Nominal MPC") # hide
ylims!(0.4,0.6) #hide
```
...
<div class="lang-switcher-content active" data-lang="julia"><pre><code class="language-julia">mpc_robust = LinearMPC.MPC(F, G; Ts=0.1, Np=25, C=[1 0;])
...
plot!(sim_robust.ys[1,:], label="Robust MPC")</code></pre></div>
<div class="lang-switcher-content" data-lang="python"><pre><code class="language-python">mpc_robust = MPC(F, G, C=np.array([[1, 0]]))
...
```@example robust_mpc
mpc_robust = LinearMPC.MPC(F,G;Ts=0.1,Np=25,C=[1 0;]) # hide
set_prestabilizing_feedback!(mpc_robust) # hide
set_disturbance!(mpc_robust,[-0.005;-0.005],[0.005;0.005]) # hide
set_bounds!(mpc_robust;umin=[-0.2],umax=[0.2],ymin=[-0.5],ymax=[0.5]) # hide

sim_robust = Simulation(true_dynamics,mpc_robust;r=[0.5]) # hide
plot!(sim_robust.ys[1,:], label="Robust MPC") # hide
ylims!(0.4,0.6) #hide
```
...
<div class="lang-switcher-content active" data-lang="julia"><pre><code class="language-julia">worst_case_dynamics = (x, u, d) -> F*x + G*u + 0.005*ones(2)
...
plot!(sim_robust_wc.ys[1,:],  label="Robust MPC")</code></pre></div>
<div class="lang-switcher-content" data-lang="python"><pre><code class="language-python">worst_case_dynamics = lambda x, u, d: F @ x + G @ u + 0.005 * np.ones(2)
...
```@example robust_mpc
worst_case_dynamics = (x,u,d) -> F*x + G*u+0.005*ones(2); # hide
sim_nominal_wc = Simulation(worst_case_dynamics,mpc_nominal;r=[0.5]) # hide
sim_robust_wc = Simulation(worst_case_dynamics,mpc_robust;r=[0.5]) # hide
hline([0.5],label="Constraint bound", linestyle=:dash) # hide
plot!(sim_nominal_wc.ys[1,:],xlabel="Time step", ylabel="Position [m]",label="Nominal MPC") # hide
plot!(sim_robust_wc.ys[1,:], label="Robust MPC") # hide
ylims!(0.4,0.6) #hide
```
