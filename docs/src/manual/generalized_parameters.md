...
set_objective!(mpc; Q=[0.05], E=[1.0;;])
setup!(mpc)
# python
...
mpc.set_objective(Q=[0.05], E=[[1.0]])
mpc.setup()
```
...
Using generalized parameters keeps the economic term in the same modeling framework as parameterized constraints, so one preview signal `p` can drive both.
